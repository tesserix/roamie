...
export function RecordButton({ phase, onPress }: { phase: RecordingPhase; onPress: () => void }) {
  ...
  return (
    <Pressable
      ...
      <View style={{ padding: 5, borderRadius: 48, borderWidth: 1, borderColor: listening ? c.danger : c.accent, backgroundColor: c.surface }}>
        <View style={{ width: 68, height: 68, borderRadius: 34, alignItems: 'center', justifyContent: 'center', backgroundColor: listening ? c.danger : c.accent }}>
          {busy ? <ActivityIndicator color={c.onAccent} accessible={false} /> : <FontAwesome name={listening ? 'stop' : 'microphone'} size={28} color={c.onAccent} accessible={false} />}
        </View>
      </View>
