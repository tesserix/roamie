...
export const TripMemoryCard = forwardRef<View, { trip: Trip; day: number; album?: boolean }>(function TripMemoryCard({ trip, day, album }, ref) {
  ...
  return <View ref={ref} collapsable={false} style={{ backgroundColor: '#f8f2fa', borderColor: '#d9c7e6', borderWidth: 1, padding: 24, gap: 18, borderRadius: 18 }}>
    ...
    {album ? <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>{trip.photos.map((p, i) => <View key={p.id} style={{ width: '30%', padding: 4, backgroundColor: 'white', transform: [{ rotate: `${i % 2 ? 2 : -2}deg` }] }}><Image source={p.uri} style={{ width: '100%', height: 100 }} contentFit="cover" accessibilityLabel={p.caption || `Memory ${i + 1}`} /></View>)}</View> : current.stops.map(stop => <View key={stop.id} style={{ backgroundColor: 'white', borderRadius: 12, padding: 14, gap: 5 }}><Text style={{ color: '#a07bb4', fontSize: 12 }}>{stop.time} · {stop.minutes} MIN</Text><Text style={{ color: '#4d3c5d', fontFamily: serif, fontSize: 20 }}>{stop.title}</Text><Text style={{ color: '#796c80', fontSize: 12 }}>{stop.note}</Text><Text style={{ color: '#79528e', fontSize: 12 }}>Est. {format(stop.costMinor, trip.currency)}</Text></View>)}
